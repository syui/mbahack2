...
var talaria = (function (P) {
    ...
    var wrapperTemplate = function(id, url, ccount, commentsHidden) {
        ...
        wrapper.innerHTML =
	    '<style>.btn::before, .btn::after { display: none;} .btn.btn-secondary.btn-lg.btn-block:hover { background-color: rgba(0, 0, 0, 0.04); color: #428bca; } </style>' +
            ...
            '    <a id="talaria-show-' + id + '" href="' + url + '" target="_blank">' + (ccount === 0 ? '<button type="button" class="btn btn-secondary btn-lg btn-block" style="border: 1px solid !important;" >Comment</button>' : (ccount + ' comment' + (ccount === 1 ? '' : 's'))) + '</a>' +
            '  </div>' +
